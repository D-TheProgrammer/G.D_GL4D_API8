...
static GLuint _pId_transition, _pId_lave ,_pId_TV, _pId_blanc ,_pId_rouge , _pId_blanc2 , _pId_blancObstacle , _pId_tissue
=0;
...
static void init(void) {
  ...
  _pId_tissue = gl4duCreateProgram("<vs>shaders/basic.vs", "<fs>shaders/basic_TISSUE.fs", NULL);
...
//cube13 sol (jaune) anti gravite vertical 
  ...
  /* envoi de toutes les matrices stockées par GL4D */
  useNoiseTextures(_pId_tissue, 0);
  gl4duSendMatrices();
  /* dessiner une première fois la sphère avec le _pId */
  gl4dgDraw(_cubeId);
  unuseNoiseTextures(0);
